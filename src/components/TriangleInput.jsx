...
export default function TriangleInput({ mode, onModeChange, rightAngle, onRightAngleChange, sides, catheti, errors, onSidesChange, onCathetiChange }) {
  return (
    <div className="space-y-2">
      <label className="block text-sm">Вид треугольника
        <select className="mt-1 w-full rounded border p-2" value={mode} onChange={(e) => onModeChange(e.target.value)}>
          <option value="sides">Произвольный (по трём сторонам)</option>
          <option value="catheti">Прямоугольный (по катетам)</option>
        </select>
      </label>
